...
namespace FinalProjectHMS
{
    public partial class RemovePatient : Form
    {
        ...
        private void RemovePatient_Load(object sender, EventArgs e)
        {
            sidebarTimer.Start();
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    con.Open();
                    string stm = "SELECT * FROM PatientData WHERE NIC = @NIC OR Mobile = @Mobile";

                    using (SqlCommand cmd = new SqlCommand(stm, con))
                    {
                        // Set parameter values
                        cmd.Parameters.AddWithValue("@NIC", txt_PatientNIC.Text);
                        cmd.Parameters.AddWithValue("@Mobile", txt_PatientMobile.Text);
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                lbl_PatientNameShow.Text = reader["PatientName"].ToString();
                                lbl_PatientNICShow.Text = reader["NIC"].ToString();
                                lbl_PatientAddressShow.Text = reader["PatientAddress"].ToString();
                                lbl_PatientBirthdayShow.Text = ((DateTime)reader["Birthday"]).ToShortDateString();
                                lbl_patientMobileShow.Text = reader["Mobile"].ToString();
                            }
                            else
                            {
        ...
        private void btn_viewAppointments_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(CS))
                {
                    ...
                    string Mobile = txt_PatientMobile.Text;
                    string stm = "DELETE FROM PatientData WHERE NIC = @NIC OR Mobile = @Mobile";
