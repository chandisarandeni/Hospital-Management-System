...
namespace FinalProjectHMS
{
    public partial class DoctorDashboard : Form
    {
        ...
        private void DoctorDashboard_Load(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string query = "SELECT AppointmentDate, PatientName FROM appointmentAndPrescriptions WHERE DoctorName = 'Dr. Sampath Fernando'";
            ...
            //Show Upcoming Appointments
            string connectionString6 = "Data Source=DESKTOP-3RD2OGB;Initial Catalog=CSE_FinalHMS;Integrated Security=True;Encrypt=True;TrustServerCertificate=True";

            using (SqlConnection conn = new SqlConnection(connectionString6))
            {
                conn.Open();
                string query = "SELECT PatientName, Prescription FROM appointmentAndPrescriptions WHERE DoctorName = 'Dr. Sampath Fernando'";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        grid_PatientAppointments.DataSource = table;
                    }
                }
            }
